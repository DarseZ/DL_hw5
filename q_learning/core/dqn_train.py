...
class DQNTrain(QNTrain):
    ...
    def forward_loss(
        ...
        gamma = torch.Tensor(np.tile(self.config.gamma, num_batch))
        qsamp_term2 = self.target_q_net(next_state)
        qsamp_term2 = torch.max(qsamp_term2, dim=1)[0]
        qsamp = reward + qsamp_term2*(1-done_mask)*gamma
    ...
    def update_target_params(self):
        ...
        #####################################################################
        my_state_dict = self.q_net.state_dict()
        self.target_q_net.load_state_dict(my_state_dict)
        #####################################################################
        #                             END OF YOUR CODE                      #
        #####################################################################
    ...
    def update_step(self, t, replay_buffer, lr):
        ...
        next_state = self.process_state(sp_batch).to(self.device)
        done_masks = torch.FloatTensor(done_mask_batch).to(self.device)

        # step2:
        q_loss = self.forward_loss(states, actions, rewards, next_state, done_masks)
